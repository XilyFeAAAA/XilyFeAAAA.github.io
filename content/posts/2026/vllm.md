...
经过 Grok 的提醒，我把 Continuous Batching 和 Chunked Prefill 混在一起了。Continuous Batching 指的是<mark>动态插入、token-level 更新、避免 sequence-level 等待</mark>。我们在 token level 进行 forward pass，每进行一个 step 就判断是否有新的 sequence 需要 Prefill，因为 Prefill 是技术密集型优先级高。而 Chunked Prefill 做的才是在同一个 forward pass 里同时跑 Prefill + Decode。
