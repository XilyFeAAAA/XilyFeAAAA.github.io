...
tags:
  - 大模型
  - 强化学习
lastmod: 2026-06-07T10:59:02+08:00
---
## 公式推导
...
我们希望最大化策略在奖励函数下的期望回报，同时用 KL 散度约束新策略不偏离参考模型太远，$\beta$ 控制两者的权衡。

---

DPO 作者的思路是：把优化问题转化成最小化某个 KL 散度，因为KL散度有一个完美的性质：

$$
D_{KL}[p \| q] \geq 0，等号当且仅当 p = q
$$

所以如果我们能把目标写成 $\min_\pi D_{KL}[\pi \| q^*]$，那最优解就是 $\pi = q^*$。所以作者对前面的优化目标进行了一系列的变化：
...
这个表达式的结构已经非常接近 KL 散度 $D_{KL}[\pi \| q]$，但分母 $\sum_y \pi_{\text{ref}}(y|x)\exp(\tfrac{1}{\beta}r_\phi(x,y)) \neq 1$ ，它不是一个合法概率分布。所以我们希望对分母进行归一化处理，使它满足 $\sum_y q(y|x)=1$。
...
这个优化目标我们很容易很观察到，当 KL 期望取最小值时，自然有 $\pi$ = $\pi^*$，因此**最优策略的闭合解**为：
...
- 同时回顾最开始我们的目标：省略训练奖励模型这个步骤，一步到位来训练对齐模型。而目前我们得到的 $Z(x)$ 的显式解**仍依赖显式奖励函数 $r$。** DPO 的核心目标是省去单独训练 Reward Model 的步骤，而闭合解仍然要求已知 $r$，与目标相悖。

DPO 从最优策略的表达式出发，对两边取对数并整理，可以反推出奖励函数的等价表达：
